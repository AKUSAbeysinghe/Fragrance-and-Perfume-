...
import Womens from "../../assets/SubCategories/womens.jpg";
...
const WomensPerfumes = () => {
  ...
  // Subcategory filters for Women's Perfumes (Category 1)
  const SUBCATEGORY_IDS = {
    ALL: "all",
    EDP: "edp",           // Eau de Parfum
    EDT: "edt",           // Eau de Toilette
    EXTRAIT: "extrait",   // Parfum / Extrait
    FLORAL: "floral",
    FRUITY: "fruity",
    WOODY: "woody",
    ORIENTAL: "oriental",
  ...
  const fetchPerfumes = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const perfumeItems = data.data.filter(item => Number(item.category_id) === 1);
      ...
      <section className="relative w-full bg-[#0a0908] text-[#e5d5be] py-20 px-6 md:py-28 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 md:space-y-8">
          ...
          <p className="text-sm md:text-base font-light tracking-wide leading-relaxed text-[#bda073]/80 max-w-2xl mx-auto">
            Exquisite fragrances crafted in the tradition of Grasse. 
            Each composition is a sensory journey — from delicate florals to deep orientals.
          </p>
        </div>
      </section>

      {/* Filters - Elegant Dark Theme */}
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {[
            ...
            { label: "Parfum / Extrait", value: SUBCATEGORY_IDS.EXTRAIT },
            { label: "Floral", value: SUBCATEGORY_IDS.FLORAL },
            { label: "Fruity", value: SUBCATEGORY_IDS.FRUITY },
            { label: "Woody", value: SUBCATEGORY_IDS.WOODY },
            { label: "Oriental", value: SUBCATEGORY_IDS.ORIENTAL },
          ].map((filter) => (
            <button
              key={filter.value}
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            ...
            <p className="col-span-full text-center py-24 text-[#bda073]/70">No perfumes found in this category.</p>
          ) : (
      ...
      <section className="bg-[#0a0705] text-[#e4d7cd] px-6 py-20 md:py-28 border-t border-stone-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-[#bda073]/80 leading-relaxed max-w-2xl mx-auto mb-16">
            From the rose fields of Grasse to the deep resins of the East, 
            each perfume is a carefully composed narrative in liquid form.
          </p>

          <div className="grid grid-cols-3 gap-8 max-w-md mx-auto text-center">
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">30</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">YEARS OF EXPERTISE</div>
            </div>
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">8</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">SIGNATURE NOTES</div>
            </div>
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">100%</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">NATURAL ESSENCES</div>
            </div>
          </div>
        </div>
      </section>
