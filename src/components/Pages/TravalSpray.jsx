...
import TravelSpray from "../../assets/SubCategories/TravelSpray.jpg";

const TravelMiniSizes = () => {
  ...
  // Subcategory filters for Travel & Mini Sizes (Category 7)
  const SUBCATEGORY_IDS = {
    ALL: "all",
    MINI_BOTTLES: "mini-bottles",
    ROLLERBALL: "rollerball",
    TRAVEL_SPRAYS: "travel-sprays",
  };
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const items = data.data.filter(item => Number(item.category_id) === 7);
        setAllProducts(items);
        setFilteredProducts(items);
      } else {
        setError("Failed to load travel & mini sizes");
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-[#0a0705] text-[#e4d7cd] font-sans min-h-screen">
      ...
      <section className="relative w-full bg-[#0a0908] text-[#e5d5be] py-20 px-6 md:py-28 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 md:space-y-8">
          ...
          <h1 className="font-serif text-5xl md:text-6xl lg:text-7xl font-light leading-none tracking-wide text-[#f4e0a5]">
            Travel &amp;{' '}
            <span className="italic font-normal text-[#c9a063]">Mini Sizes</span>
          </h1>
          ...
          <p className="text-sm md:text-base font-light tracking-wide leading-relaxed text-[#bda073]/80 max-w-2xl mx-auto">
            Portable luxury. Perfect miniatures and travel sizes to take your signature scent wherever you go.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {[
            { label: "All Travel &amp; Minis", value: "all" },
            { label: "Mini Bottles", value: SUBCATEGORY_IDS.MINI_BOTTLES },
            { label: "Rollerball Perfumes", value: SUBCATEGORY_IDS.ROLLERBALL },
            { label: "Travel Sprays", value: SUBCATEGORY_IDS.TRAVEL_SPRAYS },
          ].map((filter) => (
            <button
              key={filter.value}
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            ...
            <p className="col-span-full text-center py-24 text-[#bda073]/70">No travel &amp; mini products found in this category.</p>
          ) : (
                  ...
                  <img
                    src={
                      product.image_url
                        ...
                        : "https://via.placeholder.com/600x600?text=Travel+Mini"
                    }
                    alt={product.name}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 brightness-90"
                    onError={(e) => {
                      e.target.src = "https://via.placeholder.com/600x600?text=Travel+Mini";
                    }}
                  />
                  {product.popular === "1" && (
                ...
                <div className="p-8">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs tracking-[0.2em] uppercase text-[#c9a063]/80 font-medium">
                      {product.sub_category_name || "Travel Size"}
                    </p>
      ...
      <div className="w-full max-w-6xl mx-auto px-6 py-12">
        <div className="relative rounded-xl overflow-hidden border border-stone-800">
          <img
            src={TravelSpray}
            alt="Travel and mini fragrance collection"
      ...
      <section className="bg-[#0a0705] text-[#e4d7cd] px-6 py-20 md:py-28 border-t border-stone-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-[#bda073]/80 leading-relaxed max-w-2xl mx-auto mb-16">
            Compact yet powerful fragrances designed for the modern traveler. 
            Your favorite scents, perfectly portioned.
          </p>

          <div className="grid grid-cols-3 gap-8 max-w-md mx-auto text-center">
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">5ml</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">MINI SIZES</div>
            </div>
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">3</div>
            ...
            <div>
              ...
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">AIRPORT FRIENDLY</div>
            </div>
          </div>
        </div>
      </section>
...
export default TravelMiniSizes;