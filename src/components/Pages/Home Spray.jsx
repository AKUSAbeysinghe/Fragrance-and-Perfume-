...
import HomeSpray from "../../assets/SubCategories/homespray.jpg";

const HomeFragrances = () => {
  ...
  // Subcategory filters for Home Fragrances (Category 5)
  const SUBCATEGORY_IDS = {
    ALL: "all",
    SCENTED_CANDLES: "scented-candles",
    REED_DIFFUSERS: "reed-diffusers",
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const items = data.data.filter(item => Number(item.category_id) === 5);
        setAllProducts(items);
        setFilteredProducts(items);
      } else {
        setError("Failed to load home fragrances");
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="bg-[#0a0705] text-[#e4d7cd] font-sans min-h-screen">
      ...
      <section className="relative w-full bg-[#0a0908] text-[#e5d5be] py-20 px-6 md:py-28 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 md:space-y-8">
          ...
          <p className="text-sm md:text-base font-light tracking-wide leading-relaxed text-[#bda073]/80 max-w-2xl mx-auto">
            Transform your space with captivating scents. 
            From elegant candles to sophisticated diffusers.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {[
            { label: "All Home Fragrances", value: "all" },
            { label: "Scented Candles", value: SUBCATEGORY_IDS.SCENTED_CANDLES },
            { label: "Reed Diffusers", value: SUBCATEGORY_IDS.REED_DIFFUSERS },
            { label: "Room Sprays", value: SUBCATEGORY_IDS.ROOM_SPRAYS },
            { label: "Wax Melts", value: SUBCATEGORY_IDS.WAX_MELTS },
          ].map((filter) => (
            <button
              key={filter.value}
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            ...
            <p className="col-span-full text-center py-24 text-[#bda073]/70">No home fragrances found in this category.</p>
          ) : (
                  ...
                  <img
                    src={
                      product.image_url
                        ...
                        : "https://via.placeholder.com/600x600?text=Home+Fragrance"
                    }
                    alt={product.name}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 brightness-90"
                    onError={(e) => {
                      e.target.src = "https://via.placeholder.com/600x600?text=Home+Fragrance";
                    }}
                  />
                  {product.popular === "1" && (
                ...
                <div className="p-8">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs tracking-[0.2em] uppercase text-[#c9a063]/80 font-medium">
                      {product.sub_category_name || "Home Fragrance"}
                    </p>
      ...
      <div className="w-full max-w-6xl mx-auto px-6 py-12">
        <div className="relative rounded-xl overflow-hidden border border-stone-800">
          <img
            src={HomeSpray} 
            alt="Home fragrances collection"
      ...
      {/* The Art of Home Fragrance Section */}
      <section className="bg-[#0a0705] text-[#e4d7cd] px-6 py-20 md:py-28 border-t border-stone-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-[#bda073]/80 leading-relaxed max-w-2xl mx-auto mb-16">
            Create an inviting atmosphere with luxurious home fragrances. 
            Timeless scents that elevate every moment.
          </p>

          <div className="grid grid-cols-3 gap-8 max-w-md mx-auto text-center">
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">40+</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">HOURS BURN TIME</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">PREMIUM OILS</div>
            </div>
          </div>
        </div>
      </section>
