...
import Mens from "../../assets/SubCategories/mens.jpg";
...
const MensPerfumes = () => {
  ...
  // Subcategory filters for Men's Perfumes (Category 2)
  ...
  const fetchPerfumes = async () => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const perfumeItems = data.data.filter(item => Number(item.category_id) === 2);
        setAllPerfumes(perfumeItems);
        setFilteredPerfumes(perfumeItems);
      } else {
        setError("Failed to load men's perfumes");
      }
    } catch (err) {
      console.error(err);
    ...
    if (activeFilter === "all") {
      ...
      const filtered = allPerfumes.filter(
        item => item.sub_category_slug === activeFilter || 
                Number(item.sub_category_id) === activeFilter
      );
    ...
    <div className="bg-[#0a0705] text-[#e4d7cd] font-sans min-h-screen">
      ...
      <section className="relative w-full bg-[#0a0908] text-[#e5d5be] py-20 px-6 md:py-28 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 md:space-y-8">
          ...
          <p className="text-sm md:text-base font-light tracking-wide leading-relaxed text-[#bda073]/80 max-w-2xl mx-auto">
            Bold, refined, and timeless fragrances for the modern man. 
            From crisp citrus to deep woody accords.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {[
            ...
            { label: "Eau de Parfum", value: SUBCATEGORY_IDS.EDP },
            { label: "Eau de Toilette", value: SUBCATEGORY_IDS.EDT },
            { label: "Woody", value: SUBCATEGORY_IDS.WOODY },
            { label: "Fresh", value: SUBCATEGORY_IDS.FRESH },
            { label: "Citrus", value: SUBCATEGORY_IDS.CITRUS },
            { label: "Spicy", value: SUBCATEGORY_IDS.SPICY },
            { label: "Aquatic", value: SUBCATEGORY_IDS.AQUATIC },
          ].map((filter) => (
            <button
              key={filter.value}
        ...
        <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {isLoading ? (
            ...
            <p className="col-span-full text-center py-24 text-[#bda073]/70">No men's perfumes found in this category.</p>
          ) : (
                  ...
                  <img
                    src={
                      perfume.image_url
                        ...
                        : "https://via.placeholder.com/600x600?text=Mens+Perfume"
                    }
                    alt={perfume.name}
                    className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 brightness-90"
                    onError={(e) => {
                      e.target.src = "https://via.placeholder.com/600x600?text=Mens+Perfume";
                    }}
                  />
                  {perfume.popular === "1" && (
                ...
                <div className="p-8">
                  <div className="flex justify-between items-start mb-4">
                    <p className="text-xs tracking-[0.2em] uppercase text-[#c9a063]/80 font-medium">
                      {perfume.sub_category_name || "Eau de Parfum"}
                    </p>
      ...
      <div className="w-full max-w-6xl mx-auto px-6 py-12">
        <div className="relative rounded-xl overflow-hidden border border-stone-800">
          <img
            src={Mens}
            alt="Men's luxury perfume collection"
      ...
      <section className="bg-[#0a0705] text-[#e4d7cd] px-6 py-20 md:py-28 border-t border-stone-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-base md:text-lg text-[#bda073]/80 leading-relaxed max-w-2xl mx-auto mb-16">
            Sophisticated compositions that balance freshness with depth. 
            Timeless scents for the man who values character and presence.
          </p>

          <div className="grid grid-cols-3 gap-8 max-w-md mx-auto text-center">
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">40</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">RARE INGREDIENTS</div>
            </div>
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">7</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">SIGNATURE ACCORDS</div>
            </div>
            <div>
              <div className="text-5xl font-serif italic text-[#c9a063]">100%</div>
              <div className="text-xs tracking-widest mt-3 text-[#bda073]/70">PREMIUM FORMULAS</div>
            </div>
          </div>
        </div>
      </section>
